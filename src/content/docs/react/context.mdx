...
- Props Drilling이 거의 없다면 상태를 부모 컴포넌트로 이동하여 공유하는 것이 권장됩니다.
</Aside>
...
- `value`로 전달한 값이 변경되면 컨텍스트로 래핑된 하위 컴포넌트가 모두 리렌더링됩니다.
- 일반적으로 기본값에 `null`을 할당하고, 컴포넌트에서 값을 계산하여 `value`로 전달합니다.
