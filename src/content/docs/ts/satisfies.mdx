---
title: satisfies 연산자를 사용한 타입 검사
...
변수의 타입을 정의하기 위해 사용할 수 있는 가장 간단한 방법입니다.
...
하지만, 제공한 타입을 이용해 타입 검사와 추론을 모두 수행하려고 하기 때문에 위와 같은 오류가 발생합니다.

이 문제를 해결하기 위해 `satisfies` 연산자를 사용할 수 있습니다.

## satisfies

이 연산자는 타입 검사와 추론을 각각 따로 수행합니다.

- 주어진 타입을 이용해 타입 검사를 수행합니다.
- 값을 최대한 구체적인 타입으로 추론합니다.
...
- 값을 가장 구체적인 타입으로 추론하므로 반환되는 타입은 리터럴 타입이라고 생각할 수도 있습니다.
- 객체의 프로퍼티는 변경이 가능하므로 리터럴 타입보다 한 단계 더 넓은 기본 타입으로 추론됩니다.
- 다음과 같이 객체의 프로퍼티를 읽기 전용으로 만들면 `satisfies` 연산자는 값을 리터럴 타입으로 추론합니다.
